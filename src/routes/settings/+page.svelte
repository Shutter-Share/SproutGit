<script lang="ts">
  ...
  function findToolDisplay(value: string): ToolDisplay | null {
    if (!value.trim()) return null;
    const token = commandToken(value);
    const match = gitTools.find(
      tool => tool.id === value || tool.id === token || tool.command === token
    );
    if (match) return { id: match.id, name: match.name };
    return fallbackDisplay(value);
  }
  ...
  Promise.all([
    ...
    .then(
      ([
        ...
        if (
          configuredEditor &&
          !detectedEditors.some(e => e.installed && matchesEditor(e, configuredEditor))
        ) {
...
<main class="sg-body flex h-screen flex-col" data-testid="settings-page">
  ...
  >
    <button
      ...
      ><span class="transition-transform group-hover:-translate-x-0.5">←</span> <span>Projects</span></button
    >
  ...
  <div class="flex-1 overflow-auto p-6">
    <div class="mx-auto flex max-w-6xl flex-col gap-6">
      <div class="relative flex items-center gap-3 rounded-lg bg-gradient-to-r from-(--sg-primary)/8 via-(--sg-primary)/4 to-transparent px-4 py-3">
        <span aria-hidden="true" class="absolute top-3 bottom-3 left-0 w-[3px] rounded-r-full bg-(--sg-primary)"></span>
        <span class="flex h-9 w-9 items-center justify-center rounded-md bg-(--sg-primary)/12 text-(--sg-primary)">
          <Settings size={18} />
        </span>
        <div>
          <h1 class="sg-heading text-lg font-semibold text-(--sg-primary)">Settings</h1>
          <p class="text-[11px] text-(--sg-text-faint)">Configure SproutGit, integrations, and your default tools.</p>
        </div>
      </div>
      ...
      <section class="rounded-lg border border-(--sg-border) bg-(--sg-surface) p-5">
        <div class="mb-3 flex items-center gap-2">
          <svg
            viewBox="0 0 16 16"
            aria-hidden="true"
            class="h-4 w-4 text-(--sg-primary)"
            fill="none"
            stroke="currentColor"
            stroke-width="1.4"
            stroke-linecap="round"
            stroke-linejoin="round"
            ><circle cx="3.25" cy="8" r="1.5" /><circle cx="12.75" cy="4" r="1.5" /><circle
              cx="12.75"
              cy="12"
              r="1.5"
            /><path d="M4.6 7.2 11.4 4.8" /><path d="M4.6 8.8 11.4 11.2" /></svg
          >
          <h2 class="sg-heading text-sm font-semibold text-(--sg-primary)">Git Provider</h2>
        </div>
        {#if githubAuth === null}
        ...
        {:else if githubAuth.authenticated}
          <div
            class="flex items-center justify-between rounded border border-(--sg-border) bg-(--sg-surface-raised) px-3 py-2.5"
          >
            <p class="text-xs text-(--sg-text)">{githubAuth.username}</p>
            <button
              class="rounded border border-(--sg-border) px-3 py-1.5 text-xs text-(--sg-text-dim) hover:border-(--sg-danger) hover:text-(--sg-danger)"
        ...
        {:else}
          <button
            class="inline-flex items-center gap-2 rounded border border-(--sg-border) bg-(--sg-surface-raised) px-3 py-1.5 text-xs text-(--sg-text) hover:bg-(--sg-primary) hover:text-white"
            onclick={startGithubLogin}
            disabled={authStarting}
            ><svg viewBox="0 0 16 16" aria-hidden="true" class="h-3.5 w-3.5" fill="currentColor"
              ><path
                d="M8 0C3.58 0 0 3.67 0 8.2c0 3.62 2.29 6.7 5.47 7.78.4.08.55-.18.55-.39 0-.2-.01-.85-.01-1.54-2.23.5-2.7-.98-2.7-.98-.36-.95-.9-1.2-.9-1.2-.73-.52.06-.51.06-.51.8.06 1.23.84 1.23.84.72 1.26 1.88.9 2.34.69.07-.54.28-.9.5-1.1-1.78-.21-3.64-.91-3.64-4.06 0-.9.31-1.63.82-2.2-.09-.21-.36-1.04.08-2.17 0 0 .67-.22 2.2.84A7.43 7.43 0 0 1 8 3.46c.68 0 1.37.1 2.01.31 1.53-1.06 2.2-.84 2.2-.84.44 1.13.17 1.96.08 2.17.51.57.82 1.3.82 2.2 0 3.16-1.87 3.84-3.66 4.05.29.26.54.77.54 1.56 0 1.13-.01 2.03-.01 2.31 0 .21.14.47.55.39A8.2 8.2 0 0 0 16 8.2C16 3.67 12.42 0 8 0Z"
              /></svg
      ...
      <div class="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
        <section class="rounded-lg border border-(--sg-border) bg-(--sg-surface)">
          <div class="border-b border-(--sg-border) px-5 py-4">
            <div class="flex items-center gap-2">
              <GitBranch size={16} class="text-(--sg-primary)" />
              <h2 class="sg-heading text-sm font-semibold text-(--sg-primary)">Git Settings</h2>
            </div>
            <p class="mt-1 text-xs text-(--sg-text-faint)">
              These update your global Git configuration.
            </p>
          </div>

          {#if toolsLoading}
            <div class="px-5 py-5 text-xs text-(--sg-text-dim)">
          ...
          {:else}
            <div class="divide-y divide-(--sg-border)">
              <div class="px-5 py-4">
                <div class="flex items-start justify-between">
                  <div class="flex gap-2">
                    <User size={14} class="mt-0.5 text-(--sg-text-dim)" />
                    <div>
                      <p class="sg-heading text-xs font-semibold text-(--sg-text)">
                        Author Identity
                      </p>
                  ...
                  <button
                    class="inline-flex items-center gap-1 rounded border border-(--sg-border) px-2.5 py-1 text-xs text-(--sg-text-dim)"
                    onclick={() => togglePanel('author')}
                    ><Pencil size={12} /> {editingAuthor ? 'Done' : 'Edit'}</button
                  >
                </div>
                {#if editingAuthor}
                  <div class="mt-3 space-y-2 border-t border-(--sg-border) pt-3">
                    <input
                      bind:value={customGitName}
                      class="w-full rounded border border-(--sg-input-border) bg-(--sg-input-bg) px-2.5 py-1.5 text-xs text-(--sg-text)"
                      placeholder="Git user.name"
                    />
                    <input
                      bind:value={customGitEmail}
                      class="w-full rounded border border-(--sg-input-border) bg-(--sg-input-bg) px-2.5 py-1.5 text-xs text-(--sg-text)"
                      placeholder="Git user.email"
                    />
                    <div class="flex flex-wrap gap-2">
                      <button
                        class="rounded border border-(--sg-border) px-3 py-1.5 text-xs text-(--sg-text)"
                        onclick={saveGitIdentity}>Save Author</button
                      >{#if githubAuth?.authenticated && githubAuth.username}<button
                          class="rounded border border-(--sg-border) px-3 py-1.5 text-xs text-(--sg-text)"
                          onclick={applyGithubUsernameAsAuthor}>Use GitHub Username</button
                        >{/if}
                    </div>
                    {#if githubAuth?.authenticated}
                      <div class="flex flex-wrap gap-2">
                        {#if githubEmailsLoading}<span class="text-[11px] text-(--sg-text-faint)"
                            >Loading GitHub emails...</span
                          >{:else}{#each githubEmailSuggestions as suggestion}<button
                              class="rounded border border-(--sg-border) px-2 py-1 text-[11px] text-(--sg-text-dim)"
                              onclick={() => applyGithubEmail(suggestion)}
                ...
                <div class="flex items-start justify-between">
                  <div class="flex gap-2">
                    <svg
                      viewBox="0 0 16 16"
                      aria-hidden="true"
                      class="mt-0.5 h-3.5 w-3.5 text-(--sg-text-dim)"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="1.4"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      ><path d="M3 13.5h10" /><path d="M5 11.5h6" /><path
                        d="M4.5 2.5h7l1 1v5l-1 1h-7l-1-1v-5z"
                      /></svg
                    >
                    <div>
                      <p class="sg-heading text-xs font-semibold text-(--sg-text)">Editor</p>
                      {#if editorDisplay}<p class="text-[11px] text-(--sg-text-faint)">
                          {editorDisplay.name}
                        </p>{:else}<p class="text-[11px] text-(--sg-text-faint)">(not set)</p>{/if}
                    </div>
                  </div>
                  <button
                    class="inline-flex items-center gap-1 rounded border border-(--sg-border) px-2.5 py-1 text-xs text-(--sg-text-dim)"
                    onclick={() => togglePanel('editor')}
                    ><Pencil size={12} /> {editingEditor ? 'Done' : 'Edit'}</button
                  >
                </div>
                {#if editingEditor}
                  <div class="mt-3 space-y-2 border-t border-(--sg-border) pt-3">
                    <div class="flex flex-wrap gap-2">
                      {#each installedEditors as editor}<button
                          class="rounded border px-3 py-1.5 text-xs {currentEditor &&
                          matchesEditor(editor, currentEditor)
                            ? 'border-(--sg-primary) text-(--sg-primary)'
                            : 'border-(--sg-border) text-(--sg-text-dim)'}"
                          onclick={() => selectEditor(editor)}>{editor.name}</button
                        >{/each}
                    </div>
                    <div class="flex gap-2">
                      <input
                        bind:value={customEditor}
                        class="min-w-0 flex-1 rounded border border-(--sg-input-border) bg-(--sg-input-bg) px-2.5 py-1.5 font-mono text-xs text-(--sg-text)"
                    ...
                    {#if unavailableEditors.length > 0}<p
                        class="text-[11px] text-(--sg-text-faint)"
                      >
                ...
                <div class="flex items-start justify-between">
                  <div class="flex gap-2">
                    <svg
                      ...
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      ><path d="M2.5 4.5h11" /><path d="M2.5 11.5h11" /><circle
                        cx="5"
                        cy="4.5"
                        r="1.25"
                        fill="currentColor"
                        stroke="none"
                      /><circle cx="11" cy="11.5" r="1.25" fill="currentColor" stroke="none" /></svg
                    >
                    <div>
                      <p class="sg-heading text-xs font-semibold text-(--sg-text)">Diff Tool</p>
                      {#if diffToolDisplay}<p class="text-[11px] text-(--sg-text-faint)">
                  ...
                  <button
                    ...
                    ><Pencil size={12} /> {editingDiffTool ? 'Done' : 'Edit'}</button
                  >
                </div>
                {#if editingDiffTool}
                  <div class="mt-3 space-y-2 border-t border-(--sg-border) pt-3">
                    <div class="flex flex-wrap gap-2">
                      {#each installedDiffTools as tool}<button
                          class="rounded border px-3 py-1.5 text-xs {currentDiffTool === tool.id
                            ? 'border-(--sg-primary) text-(--sg-primary)'
                            : 'border-(--sg-border) text-(--sg-text-dim)'}"
                          onclick={() => applyDetectedDiffTool(tool)}>{tool.name}</button
                        >{/each}
                    </div>
                    <div class="flex gap-2">
                      <input
                        bind:value={customDiffTool}
                        class="min-w-0 flex-1 rounded border border-(--sg-input-border) bg-(--sg-input-bg) px-2.5 py-1.5 font-mono text-xs text-(--sg-text)"
                        placeholder="Custom diff.tool"
                      /><button
                        class="rounded border border-(--sg-border) px-3 py-1.5 text-xs text-(--sg-text)"
                        onclick={saveCustomDiffTool}>Save</button
                      >
                    </div>
                  </div>
                {/if}
                ...
                <div class="flex items-start justify-between">
                  <div class="flex gap-2">
                    <svg
                      ...
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      ><path d="M5 3v6.5" /><path d="M11 13V6.5" /><path
                        d="M2.5 6.5 5 9l2.5-2.5"
                      /><path d="M8.5 9.5 11 7l2.5 2.5" /></svg
                    >
                    <div>
                      <p class="sg-heading text-xs font-semibold text-(--sg-text)">Merge Tool</p>
                      {#if mergeToolDisplay}<p class="text-[11px] text-(--sg-text-faint)">
                  ...
                  <button
                    ...
                    ><Pencil size={12} /> {editingMergeTool ? 'Done' : 'Edit'}</button
                  >
                </div>
                {#if editingMergeTool}
                  <div class="mt-3 space-y-2 border-t border-(--sg-border) pt-3">
                    <div class="flex flex-wrap gap-2">
                      {#each installedMergeTools as tool}<button
                          class="rounded border px-3 py-1.5 text-xs {currentMergeTool === tool.id
                            ? 'border-(--sg-primary) text-(--sg-primary)'
                            : 'border-(--sg-border) text-(--sg-text-dim)'}"
                          onclick={() => applyDetectedMergeTool(tool)}>{tool.name}</button
                        >{/each}
                    </div>
                    <div class="flex gap-2">
                      <input
                        bind:value={customMergeTool}
                        class="min-w-0 flex-1 rounded border border-(--sg-input-border) bg-(--sg-input-bg) px-2.5 py-1.5 font-mono text-xs text-(--sg-text)"
                        placeholder="Custom merge.tool"
                      /><button
                        class="rounded border border-(--sg-border) px-3 py-1.5 text-xs text-(--sg-text)"
                        onclick={saveCustomMergeTool}>Save</button
                      >
                    </div>
                  </div>
                {/if}
              ...
              <div class="px-5 py-4">
                <div class="flex items-center gap-2">
                  <Info size={14} class="text-(--sg-text-dim)" />
                  <p class="sg-heading text-xs font-semibold text-(--sg-text)">Git Installation</p>
                </div>
                {#if gitInfo === null}<p class="mt-1 text-xs text-(--sg-text-faint)">
                    Checking...
                  </p>{:else if gitInfo.installed}<p class="mt-1 text-xs text-(--sg-text-dim)">
                    {gitInfo.version}
                  </p>{:else}<p class="mt-1 text-xs text-(--sg-danger)">Git not found</p>{/if}
              </div>
            </div>
          {/if}
        </section>
        ...
        <div class="space-y-6">
          <section class="rounded-lg border border-(--sg-border) bg-(--sg-surface) p-5">
            <div class="mb-2 flex items-center gap-2">
              <SquareTerminal size={16} class="text-(--sg-primary)" />
              <h2 class="sg-heading text-sm font-semibold text-(--sg-primary)">Terminal Shell</h2>
            </div>
            <p class="mb-3 text-xs text-(--sg-text-faint)">
              Default shell used in SproutGit's terminal panel.
            </p>
            {#if shellsLoading}
              <div class="text-xs text-(--sg-text-dim)">
            ...
            {:else}
              <div class="flex flex-wrap gap-2">
                {#each availableShells as shell}<button
                    class="rounded border px-3 py-1.5 text-xs {currentShell === shell
                      ? 'border-(--sg-primary) text-(--sg-primary)'
                      : 'border-(--sg-border) text-(--sg-text-dim)'}"
                    onclick={() => selectShell(shell)}>{shell}</button
                  >{/each}
              </div>
            {/if}
          </section>
          ...
          <section class="rounded-lg border border-(--sg-border) bg-(--sg-surface) p-5">
            <div class="mb-2 flex items-center gap-2">
              <Info size={16} class="text-(--sg-primary)" />
              <h2 class="sg-heading text-sm font-semibold text-(--sg-primary)">About</h2>
            </div>
            <div class="flex items-center justify-between">
              <span class="sg-logo-text text-xs text-(--sg-text)">SproutGit</span><span
                class="font-mono text-xs text-(--sg-text-dim)"
                >{#if appVersion !== null}{import.meta.env.DEV
                    ? appVersion
                    : `v${appVersion}`}{:else}<Spinner size="sm" />{/if}</span
              >
            </div>
            <div class="mt-3 border-t border-(--sg-border) pt-3">
              {#if !updaterEnabled}
                <p class="mb-2 text-xs text-(--sg-text-faint)">
              ...
              {:else}
                ...
                <button
                  class="rounded border border-(--sg-border) bg-(--sg-surface-raised) px-3 py-1.5 text-xs text-(--sg-text)"
