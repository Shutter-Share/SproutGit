...
<main class="flex h-screen flex-col">
  ...
  <header data-tauri-drag-region class="flex shrink-0 items-center justify-between border-b border-[var(--sg-border)] bg-[var(--sg-surface)] pt-1 pr-1 pb-1 pl-[var(--sg-titlebar-inset)]">
    <span class="sg-logo-text flex items-center gap-1.5 text-sm font-semibold text-[var(--sg-text)]">
  ...
  <div class="flex min-h-0 flex-1">
    <!-- Left: Start sidebar -->
    <section class="flex w-64 shrink-0 flex-col border-r border-[var(--sg-border)] bg-[var(--sg-surface)]">
      <div class="border-b border-[var(--sg-border-subtle)] px-4 py-3">
        <h2 class="flex items-center gap-1.5 text-xs font-semibold uppercase leading-none tracking-wider text-[var(--sg-text-dim)]"><Play size={14} strokeWidth={2.5} />Start</h2>
      </div>
      ...
      {#if appVersion}
        <div class="mt-auto px-4 pb-4 pt-2 text-center">
          <p class="text-[10px] text-[var(--sg-text-faint)]">SproutGit {import.meta.env.DEV ? appVersion : `v${appVersion}`}</p>
        </div>
      {/if}
    </section>
    ...
    <section class="flex min-w-0 flex-1 flex-col">
      <div class="border-b border-[var(--sg-border-subtle)] bg-[var(--sg-surface)] px-4 py-3">
        <h2 class="flex items-center gap-1.5 text-xs font-semibold uppercase leading-none tracking-wider text-[var(--sg-text-dim)]"><Clock size={14} strokeWidth={2.5} />Recent projects</h2>
      </div>
    ...
    >
      ...
      >
        ...
        <form onsubmit={startNewProject} class="flex flex-col gap-4 p-4">
          <div>
            <label for="modal-repo-url" class="mb-1 flex items-center gap-1.5 text-xs text-[var(--sg-text-dim)]">
              Repository URL
          ...
          <div>
            <label for="modal-folder-name" class="mb-1 block text-xs text-[var(--sg-text-dim)]">Workspace folder name</label>
            <input
          ...
          <div>
            <label for="modal-projects-folder" class="mb-1 block text-xs text-[var(--sg-text-dim)]">Workspace parent folder</label>
          ...
          <div class="flex justify-end gap-2 border-t border-[var(--sg-border-subtle)] pt-3">
            ...
            >
              {#if creating}<Spinner size="sm" />&nbsp;Cloning…{:else}<Download size={13} strokeWidth={2} /> Clone{/if}
            </button>
          </div>
        </form>
      </div>
    ...
    >
      ...
      >
        ...
        <form onsubmit={importExistingRepo} class="flex flex-col gap-4 p-4">
          <div>
            ...
            <div class="flex gap-1.5">
              ...
              <button
                type="button"
                onclick={async () => {
                  const dir = await open({ directory: true, title: "Select git repository to import" });
                  if (dir) { importRepoPath = dir; handleImportRepoPathInput(); }
                }}
                class="shrink-0 rounded border border-[var(--sg-border)] bg-[var(--sg-surface-raised)] px-2.5 py-1.5 text-xs text-[var(--sg-text-dim)] hover:bg-[var(--sg-border)] hover:text-[var(--sg-text)]"
              >Browse</button>
            </div>
          ...
          <div>
            <p class="mb-2 text-xs font-medium text-[var(--sg-text-dim)]">How should it be imported?</p>
            <div class="flex flex-col gap-1.5">
              <button
                type="button"
                class="rounded-md border px-3 py-2.5 text-left text-xs transition-colors {importMode === 'inPlace' ? 'border-[var(--sg-primary)] bg-[var(--sg-primary)]/10' : 'border-[var(--sg-border)] hover:bg-[var(--sg-surface-raised)]'}"
                onclick={() => importMode = 'inPlace'}
              >
                <span class="font-semibold {importMode === 'inPlace' ? 'text-[var(--sg-primary)]' : 'text-[var(--sg-text)]'}">Import in place</span>
                <span class="mt-0.5 block text-[var(--sg-text-faint)]">Restructure the selected folder into a SproutGit workspace in-situ. The repo becomes the <code class="font-mono">root</code> inside the same parent directory.</span>
              </button>
              <button
                type="button"
                class="rounded-md border px-3 py-2.5 text-left text-xs transition-colors {importMode === 'move' ? 'border-[var(--sg-primary)] bg-[var(--sg-primary)]/10' : 'border-[var(--sg-border)] hover:bg-[var(--sg-surface-raised)]'}"
                onclick={() => importMode = 'move'}
              >
                <span class="font-semibold {importMode === 'move' ? 'text-[var(--sg-primary)]' : 'text-[var(--sg-text)]'}">Move to new workspace</span>
                <span class="mt-0.5 block text-[var(--sg-text-faint)]">Move the repo to a new folder and create the workspace structure around it. Original location is removed.</span>
              </button>
              <button
                type="button"
                class="rounded-md border px-3 py-2.5 text-left text-xs transition-colors {importMode === 'copy' ? 'border-[var(--sg-primary)] bg-[var(--sg-primary)]/10' : 'border-[var(--sg-border)] hover:bg-[var(--sg-surface-raised)]'}"
                onclick={() => importMode = 'copy'}
              >
                <span class="font-semibold {importMode === 'copy' ? 'text-[var(--sg-primary)]' : 'text-[var(--sg-text)]'}">Copy to new workspace</span>
                <span class="mt-0.5 block text-[var(--sg-text-faint)]">Copy the repo into a new workspace. Original is left untouched.</span>
              </button>
            </div>
          </div>
          ...
          {#if importMode === "move" || importMode === "copy"}
            <div>
              <label for="modal-import-folder-name" class="mb-1 block text-xs text-[var(--sg-text-dim)]">Workspace folder name</label>
              <input
          ...
          <div class="flex justify-end gap-2 border-t border-[var(--sg-border-subtle)] pt-3">
            ...
            <button
              ...
              class="flex items-center gap-2 rounded-md bg-[var(--sg-primary)] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[var(--sg-primary-hover)] disabled:cursor-not-allowed disabled:opacity-40"
            >
              {#if importing}<Spinner size="sm" />&nbsp;Importing…{:else}<FolderInput size={13} strokeWidth={2} /> Import{/if}
            </button>
          </div>
        </form>
      </div>
