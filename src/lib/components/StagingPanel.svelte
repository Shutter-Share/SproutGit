...
<div class="flex h-full flex-col overflow-hidden">
  ...
  >
    <span aria-hidden="true" class="absolute top-1.5 bottom-1.5 left-0 w-[2px] rounded-r-full bg-[var(--sg-primary)]"></span>
    <div class="flex items-center gap-2">
      ...
      {/if}
      {#if !loading}
        <span class="text-[10px] text-[var(--sg-text-faint)]">· {statusFiles.length === 0
            ? 'Clean'
