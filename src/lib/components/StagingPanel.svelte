<script lang="ts">
  ...
  const unstagedFiles = $derived(
    statusFiles.filter((f) => f.workTreeStatus !== " "),
  );
  ...
  {:else}
    <div class="flex min-h-0 flex-1">
      ...
      >
          ...
          <div class="border-b border-[var(--sg-border-subtle)]">
            ...
            {:else}
              {#each stagedFiles as file}
                <div
                  class="group flex items-center gap-1 px-2 py-0.5 hover:bg-[var(--sg-surface-raised)] {diffFile === file.path && diffStaged ? 'bg-[var(--sg-surface-raised)]' : ''}"
                >
                  <button
                  ...
                  >
                    ...
                    <span
                      class="truncate text-[11px] text-[var(--sg-text-dim)]"
                    >
                      {file.path}
                    </span>
                  </button>
        ...
        <div
          class="border-t border-[var(--sg-border)] px-3 py-2"
        ...
        >
          <textarea
            ...
            rows="3"
            class="w-full resize-none rounded border bg-[var(--sg-input-bg)] px-2 py-1.5 text-xs text-[var(--sg-text)] placeholder-[var(--sg-text-faint)] outline-none {commitError ? 'border-[var(--sg-danger)] focus:border-[var(--sg-danger)]' : 'border-[var(--sg-input-border)] focus:border-[var(--sg-input-focus)]'}"
          ></textarea>
          {#if commitError}
      ...
      <div class="flex min-w-0 flex-1 flex-col overflow-hidden bg-[var(--sg-bg)]">
        ...
        {:else if diffFile && diffContent}
          ...
          >
            <span
              class="rounded-sm px-1 py-px text-[9px] font-bold {diffStaged ? 'bg-[var(--sg-primary)]/15 text-[var(--sg-primary)]' : 'bg-[var(--sg-warning)]/15 text-[var(--sg-warning)]'}"
            >
